...
export function productCardHtmlServer(product: LocalProduct) {
  ...
  return `
    <div class="h-full transform hover:scale-105 transition-transform duration-300">
      <article class="group relative flex h-full flex-col border border-gray-200 rounded-lg overflow-hidden bg-white hover:shadow-lg transition-all duration-300">
        ...
        <div class="absolute right-2 top-4 z-10 flex translate-x-full flex-col gap-2 opacity-0 transition-all duration-500 ease-out group-hover:translate-x-0 group-hover:opacity-100">
          <button type="button" class="p-1.5 rounded-full shadow-lg border border-gofarm-green/20 backdrop-blur-sm hover:scale-110 transition-all duration-300 bg-white/90 text-gofarm-gray hover:bg-gofarm-green hover:text-white" aria-label="Add to wishlist">
        ...
        <button class="mt-auto w-full rounded-md border border-gofarm-green bg-white px-2 py-1.5 text-[10px] font-semibold text-gofarm-green transition-colors hover:bg-gofarm-green hover:text-white active:bg-gofarm-green active:text-white mx-2 mb-2" style="width: calc(100% - 16px)">
          Add to Cart
        </button>
      </article>
    </div>
